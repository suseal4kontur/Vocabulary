using System;
using System.Net.Http;
using System.Text.Json;
using System.Threading.Tasks;
using View.Entries;
using View.Meanings;

namespace VocabularyAPI.Client
{
    public sealed class VocabularyClient : IVocabularyClient
    {
        private readonly HttpClient httpClient;

        public VocabularyClient()
        {
            this.httpClient = new HttpClient
            {
                BaseAddress = new Uri("https://localhost:5001/")
            };
        }

        public async Task<Entry> GetEntryAsync(string lemma)
        {
            var response = await this.httpClient.GetAsync($"entries/{lemma}");

            var content = await response.Content.ReadAsStringAsync();
            return JsonSerializer.Deserialize<Entry>(content);
        }

        public async Task<EntriesList> GetEntriesAsync(
            string partOfSpeech = null,
            string form = null,
            string synonym = null,
            DateTime? fromAddedAt = null,
            DateTime? toAddedAt = null,
            int? offset = null,
            int? limit = null)
        {
            var query = string.Join(
                '&',
                $"partOfSpeech={partOfSpeech}",
                $"form={form}",
                $"synonym={synonym}",
                $"fromAddedAt={fromAddedAt}",
                $"toAddedAt={toAddedAt}",
                $"offset={offset}",
                $"limit={limit}");

            var response = await this.httpClient.GetAsync("entries?" + query);

            var content = await response.Content.ReadAsStringAsync();
            return JsonSerializer.Deserialize<EntriesList>(content);
        }

        public async Task CreateEntryAsync(EntryCreateInfo createInfo)
        {
            var stringContent = new StringContent(JsonSerializer
                .Serialize(createInfo, typeof(EntryCreateInfo)));

            var response = await httpClient.PostAsync($"entries/", stringContent);
            response.EnsureSuccessStatusCode();
        }

        public async Task UpdateEntryAsync(string lemma, EntryUpdateInfo updateInfo)
        {
            var stringContent = new StringContent(JsonSerializer
                .Serialize(updateInfo, typeof(EntryUpdateInfo)));

            var response = await httpClient.PatchAsync($"entries/{lemma}", stringContent);
            response.EnsureSuccessStatusCode();
        }

        public async Task DeleteEntryAsync(string lemma)
        {
            var response = await httpClient.DeleteAsync($"entries/{lemma}");
            response.EnsureSuccessStatusCode();
        }

        public async Task<Entry> GetEntryByMeaningAsync(string meaningId)
        {
            var response = await this.httpClient.GetAsync($"entries/{meaningId}/bymeaning");

            var content = await response.Content.ReadAsStringAsync();
            return JsonSerializer.Deserialize<Entry>(content);
        }

        public async Task CreateMeaningAsync(string lemma, MeaningCreateInfo createInfo)
        {
            var stringContent = new StringContent(JsonSerializer
                .Serialize(createInfo, typeof(MeaningCreateInfo)));

            var response = await httpClient.PostAsync($"entries/{lemma}/meanings/", stringContent);
            response.EnsureSuccessStatusCode();
        }

        public async Task UpdateMeaningAsync(
            string lemma,
            string meaningId,
            MeaningUpdateInfo updateInfo)
        {
            var stringContent = new StringContent(JsonSerializer
                .Serialize(updateInfo, typeof(MeaningUpdateInfo)));

            var response = await httpClient
                .PatchAsync($"entries/{lemma}/meanings/{meaningId}", stringContent);
            response.EnsureSuccessStatusCode();
        }

        public async Task DeleteMeaningAsync(string lemma, string meaningId)
        {
            var response = await httpClient.DeleteAsync($"entries/{lemma}/meanings/{meaningId}");
            response.EnsureSuccessStatusCode();
        }
    }
}
